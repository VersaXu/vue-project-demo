...
class TurtleSoupApiService {
  ...
  async sendChatMessage(prompt: string): Promise<string> {
    try {
      // 使用API_ENDPOINTS中的配置
      const response = await axios.post(API_ENDPOINTS.QWEN_CHAT, { prompt })
...
请确保谜题有趣且具有一定挑战性。`
      ...
      // 提取线索
      const clue1Match = result.match(/线索1：(.*?)(?:\n|$)/)
      const clue2Match = result.match(/线索2：(.*?)(?:\n|$)/)
      const clue3Match = result.match(/线索3：(.*?)(?:\n|$)/)
      
      // 收集线索
      const clues: string[] = [];
      if (clue1Match && clue1Match[1]) clues.push(clue1Match[1].trim());
      if (clue2Match && clue2Match[1]) clues.push(clue2Match[1].trim());
      if (clue3Match && clue3Match[1]) clues.push(clue3Match[1].trim());
      
      // 如果线索不足，添加默认线索
      while (clues.length < 3) {
        clues.push(`思考谜题的第${clues.length + 1}个角度`);
      }
